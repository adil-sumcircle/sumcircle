...
function shaderRenderer() {
	...
	const initialize = async ({ canvas, resizeTarget, enablePerformanceTracking: enableTracking = true, colorSpace = "p3-linear", toneMapping = "linear", context, gpu, forceFullFrameRate: forceFullFps = false, observeElement: shouldObserveElement = true }) => {
		...
		try {
			...
			if (context) {
				...
				if (gpu?.device) rendererOptions.device = gpu.device;
				renderer = new WebGPURenderer(rendererOptions);
				rendererInitSettled = false;
				rendererInitPromise = renderer.init();
				rendererInitPromise.finally(() => {
					rendererInitSettled = true;
				}).catch(() => {});
	...
	const cleanup = () => {
		...
		const disposeRenderer = (r) => {
			try {
				if (typeof r.forceContextLoss === "function") r.forceContextLoss();
			} catch (e) {}
			if (typeof r.dispose === "function") try {
				r.dispose();
			} catch (e) {}
		};
		if (renderer) {
			const rendererToDispose = renderer;
			const pendingInit = rendererInitPromise;
			if (pendingInit && !rendererInitSettled) pendingInit.finally(() => disposeRenderer(rendererToDispose)).catch(() => {});
			else disposeRenderer(rendererToDispose);
		}
